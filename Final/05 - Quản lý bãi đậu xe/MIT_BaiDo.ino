#include <Wire.h>
#include <LiquidCrystal_I2C.h>
#include <MFRC522.h>
#include <Stepper.h>

#define SS_PIN 53  // Pin SS cho RFID trên Arduino Mega
#define RST_PIN 7  // Pin RST cho RFID trên Arduino Mega
MFRC522 rfid(SS_PIN, RST_PIN);

LiquidCrystal_I2C lcd(0x27, 20, 4); // LCD 20x4 với địa chỉ I2C 0x27

const int stepsPerRevolution = 2048; // Số bước cho một vòng quay hoàn chỉnh
Stepper myStepper(stepsPerRevolution, 8, 10, 9, 11); // Chân kết nối động cơ bước
int gateStatus = 0;

char incoming_value;

int parking_slots[] = {26, 34, 28};

int control_slot1_ir_s = 4; // Cảm biến hồng ngoại lối vào 
int control_slot2_ir_s = 3; // Cảm biến hồng ngoại lối ra

String status_sensor[] = {"_","_","_"};  // trạng thái bãi đỗ xe : _ : empty , x : busy  
int total_slot; // tổng số vị trí 
int empty_slot; // số vị trí trống
const int interval = 5000;   // khoảng thời gian cửa mở (5 giây)
unsigned long previousMillis = 0;  // lưu thời gian cuối cùng cửa được mở

void setup() {
  Serial.begin(9600);
  SPI.begin();
  rfid.PCD_Init();

  total_slot = sizeof(parking_slots) / sizeof(parking_slots[0]); 
  empty_slot = sizeof(parking_slots) / sizeof(parking_slots[0]); // khi khởi động thì mọi vị trí đều trống, empty = total_slot
  for (int i = 0; i < total_slot; i++) {
    pinMode(parking_slots[i], INPUT_PULLUP);
  }  
  pinMode(control_slot1_ir_s, INPUT_PULLUP);
  pinMode(control_slot2_ir_s, INPUT_PULLUP);

  lcd.init();
  lcd.backlight();

  myStepper.setSpeed(15); // Tốc độ của động cơ bước
}

void loop() {
  // Bật tắt cần gạt từ app
  if (Serial.available() > 0) {
    incoming_value = Serial.read();
    Serial.println(incoming_value);
    if (incoming_value == '1') {
      Serial.println("SHOW_MESSAGE| Mo cong thanh cong");
      myStepper.step(0.25*stepsPerRevolution);
      previousMillis = millis();
    }
    else{
      closeGateIfNeeded();
      
    }
  }

  checkParkingSlots();   // cập nhật trạng thái bãi đỗ xe : _ : empty , x : busy  
  displayLCD();
  bool rfidStatus = readRFID();

  if (rfidStatus == true) {
      if (digitalRead(control_slot1_ir_s) == LOW ) {    // mở cửa lối vào 
          handleEntry();  
      } 

      if (digitalRead(control_slot2_ir_s) == LOW ) { // mở cửa lối ra
          handleExit();
      } 
      
      // mở cửa thất bại
      if (digitalRead(control_slot2_ir_s) == HIGH && digitalRead(control_slot1_ir_s) == HIGH) {
        LCD_fail();
      }
  }
  // đóng cửa   
  closeGateIfNeeded();
  delay(2000);
}



void checkParkingSlots() {
  String message = "PARKING_STATUS";
  empty_slot = total_slot;
  for(int i=0; i < total_slot; i++){
    if(digitalRead(parking_slots[i]) == LOW){
      status_sensor[i] = "x" ;
      message += "|1";
      empty_slot -= 1;
    }
    else{
      status_sensor[i] = "_";
      message += "|0";
    }
  }
  Serial.println(message);
}

void handleEntry() {
    if (empty_slot > 0) {
      openGateIfNeeded();

      LCD_entry();
    }
    else {
      LCD_fullslot();
    } 
}

void handleExit() {
    openGateIfNeeded();
    LCD_exit();
}

void openGateIfNeeded(){
  myStepper.step(0.25*stepsPerRevolution);
  Serial.println("GATE_STATUS|1"); 
  previousMillis = millis();

}

void closeGateIfNeeded() {
  if (previousMillis > 0 && millis() - previousMillis >= interval) {
    Serial.println("Close");
    myStepper.step(-0.25 * stepsPerRevolution);
    previousMillis = 0;  // Reset previousMillis để không quay lại ngay lập tức
    Serial.println("GATE_STATUS|0");
    Serial.println("SHOW_MESSAGE| Dong cong thanh cong");
  }
}

bool readRFID() {
  if (!rfid.PICC_IsNewCardPresent() || !rfid.PICC_ReadCardSerial()) {
    return false;
  }
  
  Serial.print("Card ID: ");
  for (byte i = 0; i < rfid.uid.size; i++) {
    Serial.print(rfid.uid.uidByte[i] < 0x10 ? " 0" : " ");
    Serial.print(rfid.uid.uidByte[i], HEX);
  }
  Serial.println();
  return true;
}
void displayLCD() {
  lcd.clear();
  lcd.setCursor(2, 0);
  lcd.print("Con trong: ");
  lcd.print(empty_slot);

  lcd.setCursor(0, 1);
  lcd.print("P1: ");
  lcd.print(status_sensor[0]);
  lcd.print(" ");
  lcd.print(status_sensor[1]);
  lcd.print(" ");
  lcd.print(status_sensor[2]);
}

void LCD_entry(){
  lcd.clear();
  lcd.setCursor(0, 1);
  lcd.print("Quet the thanh cong");
  lcd.setCursor(0, 2);
  lcd.print("Chao mung ban da den");
  Serial.println("SHOW_MESSAGE| Mo cong thanh cong");
}

void LCD_exit(){
  lcd.clear();
  lcd.setCursor(0, 1);
  lcd.print("Quet the thanh cong");
  lcd.setCursor(0, 2);
  lcd.print("Hen gap lai");
  Serial.println("SHOW_MESSAGE| Mo cong thanh cong");
}

void LCD_fullslot(){
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Het cho do xe");
  Serial.println("SHOW_MESSAGE| Het cho do xe");
}

void LCD_fail() {
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Quet the that bai");
  Serial.println("SHOW_MESSAGE| Quet the that bai");
}
